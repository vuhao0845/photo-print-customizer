...
export default function PhotoCustomizer() {
  ...
  const generateMergedImage = async (outputWidth = 2000) => {
    ...
    try {
      ...
      await Promise.all([ // Chờ cho đến khi cả ảnh và khung được tải
        new Promise((res) => (img.onload = res)),
        new Promise((res) => (frameImg.onload = res)),
      ]);
      ...
      canvas.height = height;
      const ctx = canvas.getContext('2d');
      ctx.drawImage(img, croppedAreaPixels.x, croppedAreaPixels.y, croppedAreaPixels.width, croppedAreaPixels.height, 0, 0, width, height);  // Vẽ ảnh người dùng lên canvas
      ctx.drawImage(frameImg, 0, 0, width, height);  // Vẽ khung lên canvas
      return canvas.toDataURL('image/png');  // Trả về ảnh cuối dưới dạng base64
    } catch (err) {
      console.error('Merge failed', err);
      return null;
    }
    ...
    <div className="container">
      ...
      <div>
        ...
        <div>
          {builtInFrames.concat(customFrames).map((frame) => (
            <div key={frame.id} onClick={() => setSelectedFrame(frame)}>
              <img src={frame.src} alt={frame.name} width="100" />
              <p>{frame.name}</p>
            </div>
          ))}
        </div>
